...
remaining:
  ...
  - unverified: "(prepublished) PUBLICATION.md is drafted (2026-09-25) but nothing in it is final: the gallery is not produced or ordered, the mature-content answer waits for the screenshots, the rollback target is not chosen, and the manual validations are the owner's. Thanks: a drafted comment for the original mod (2566355159, registered in WORKSHOP_COMMENTS.md as drafted, to post only once the item is public). Open questions for the owner: whether Work Studio's showcase-colony rule applies, and whether RimLogging's Covers gets this mod"
  ...
  - defect: "(published) the environment steam-production does not exist on this repository, so no required reviewer and no Steam secrets. Only the owner runs scripts/configure-environments.sh --repo vbardales/Rimworld-Ancient-Buildings-Renew and scripts/set-steam-secrets.sh --repo. No publish may be dispatched before, since GitHub would create the environment with no reviewer"
